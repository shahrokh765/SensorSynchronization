...
def clustering(points, min_k, max_k):  # weight should be added to points
    points_array = []
    for point in points:
        points_array += point
    points_array = np.array(points_array)
    sum_dist_to_centroid = np.zeros(max_k - min_k + 1, dtype=float)
    centroids = []
    k_ind = 0
    for k in range(min_k, max_k + 1):
        if len(points_array) < k:
            centroids.append(points_array)
            sum_dist_to_centroid[k_ind] = 0
        else:
            try:
                kmeans = KMeans(n_clusters=k).fit(points_array)
                centroids.append(kmeans.cluster_centers_)
                sum_dist_to_centroid[k_ind] = dist_to_centroids(points_array, kmeans.cluster_centers_, kmeans.labels_)
    ...
    if max_k - min_k + 1 < 4:
        if len(centroids[1]) == 0 or centroids[1] is None:
            if len(centroids[0]) == 0 or centroids[0] is None:
                return 0, None
            return min_k, centroids[0]
        x0, y0 = centroids[len(centroids) - 1][0]
        x1, y1 = centroids[len(centroids) - 1][1]
        if x0 == x1 and y0 == y1:
            return min_k, centroids[0]
        return max_k, centroids[len(centroids) - 1]
    k_ind = 0
    for k in range(min_k, max_k-1):  # choose appropriate k. the point where the previous and next slope are not that different
        prev_slope = abs(sum_dist_to_centroid[k_ind + 1] - sum_dist_to_centroid[k_ind])
        next_slope = abs(sum_dist_to_centroid[k_ind + 2] - sum_dist_to_centroid[k_ind + 1])
        if next_slope < 0.3 * prev_slope:
            return k+1, centroids[k_ind + 1]
        k_ind += 1
    return max_k, centroids[len(centroids) - 1]
...
if __name__ == '__main__':
    ...
    sensor_file_path = '../Localization/data50/homogeneous-100/sensors'
    ...
    for max_skew in range(min_max_skew, max_max_skew + 1, skew_step):
        ...
        for num_intruders in range(min_number_intruders, max_number_intruders + 1):
            for cluster_range in range(1, num_intruders + 1):
                print("************* Cluster Range:[" + str(num_intruders - cluster_range + 1) + " " +
                      str(num_intruders + cluster_range) + "]  ***************")
                errors_skewed = []
                misses_skewed = []
                false_alarms_skewed = []
                errors_aligned = []
                ...
                for repeat in range(1, repeats + 1):
                    ...
                    if 1 == 1:  # make it true to run splot for each observation vector
                        ...
                        k, pred_locations_splot_cluster = clustering(pred_locations_splot,
                                                                     num_intruders - cluster_range + 1,
                                                                     num_intruders + cluster_range)
                    ...
                    if 1 == 1:  # SPLOT with aligned signals
                        ...
                        k, pred_locations_algn_splot_cluster = clustering(pred_locations_algn_splot,
                                                                          num_intruders - cluster_range + 1,
                                                                          num_intruders + cluster_range)
                        print("Number of Clusters:" + str(k))
